...
(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    ? define(["chart.js", "hammerjs", "chart.js/helpers"], factory)
    : ((global =
        typeof globalThis !== "undefined" ? globalThis : global || self),
      (global.ChartZoom = factory(
        global.Chart,
        global.Hammer,
  ...
  function updateRange(scale, { min, max }, limits, zoom = false) {
    ...
    return (
      scale.parse(newRange.min) !== scale.min ||
      scale.parse(newRange.max) !== scale.max
    );
  }
  ...
  const integerChange = (v) =>
    v === 0 || isNaN(v)
      ? 0
      : v < 0
      ? Math.min(Math.round(v), -1)
      : Math.max(Math.round(v), 1);
  ...
  function panNumericalScale(scale, delta, limits, pan = false) {
    ...
    const newMin = scale.getValueForPixel(
      scale.getPixelForValue(prevStart + offset) - delta
    );
    const newMax = scale.getValueForPixel(
      scale.getPixelForValue(prevEnd + offset) - delta
    );
  ...
  function storeOriginalScaleLimits(chart, state) {
    ...
    helpers.each(scales, function (scale) {
      if (
        shouldUpdateScaleLimits(scale, originalScaleLimits, updatedScaleLimits)
      ) {
  ...
  function zoom(chart, amount, transition = "none", trigger = "api") {
    ...
    const xEnabled = x !== 1;
    const yEnabled = y !== 1;
    const enabledScales = getEnabledScalesByPoint(
      zoomOptions,
      focalPoint,
      chart
    );
  ...
  function getOriginalRange(state, scaleId) {
    ...
    return (
      helpers.valueOrDefault(max.options, max.scale) -
      helpers.valueOrDefault(min.options, min.scale)
    );
  }
  function getZoomLevel(chart) {
    ...
    helpers.each(chart.scales, function (scale) {
      ...
      if (origRange) {
        const level =
          Math.round((origRange / (scale.max - scale.min)) * 100) / 100;
  ...
  function pan(chart, delta, enabledScales, transition = "none") {
    const { x = 0, y = 0 } =
      typeof delta === "number" ? { x: delta, y: delta } : delta;
    const state = getState(chart);
    const {
  ...
  function isZoomedOrPanned(chart) {
    ...
    for (const scaleId of Object.keys(chart.scales)) {
      ...
      if (
        ...
        chart.scales[scaleId].min !== originalMin
      ) {
        return true;
      }
      if (
        originalMax !== undefined &&
        chart.scales[scaleId].max !== originalMax
      ) {
        return true;
      }
    }
  ...
  function applyMinMaxProps(rect, chartArea, points, { min, max, prop }) {
    rect[min] = clamp(
      Math.min(points.begin[prop], points.end[prop]),
      chartArea[min],
      chartArea[max]
    );
    rect[max] = clamp(
      Math.max(points.begin[prop], points.end[prop]),
      chartArea[min],
      chartArea[max]
    );
  }
  ...
  function computeDragRect(chart, mode, pointEvents, maintainAspectRatio) {
    ...
    const points = getRelativePoints(
      chart,
      pointEvents,
      maintainAspectRatio && xEnabled && yEnabled
    );
    if (xEnabled) {
  ...
  function addDebouncedHandler(chart, name, handler, delay) {
    if (handler) {
      getState(chart).handlers[name] = debounce(
        () => helpers.callback(handler, [{ chart }]),
        delay
      );
    }
  }
  ...
  function startPinch(chart, state, event) {
    if (state.options.zoom.pinch.enabled) {
      const point = helpers.getRelativePosition(event, chart);
      if (
        helpers.callback(state.options.zoom.onZoomStart, [
          { chart, event, point },
        ]) === false
      ) {
        state.scale = null;
  ...
  var Zoom = {
    ...
    start: function (chart, _args, options) {
      ...
      chart.pan = (delta, panScales, transition) =>
        pan(chart, delta, panScales, transition);
      chart.zoom = (args, transition) => zoom(chart, args, transition);
      chart.zoomRect = (p0, p1, transition) =>
        zoomRect(chart, p0, p1, transition);
      chart.zoomScale = (id, range, transition) =>
        zoomScale(chart, id, range, transition);
